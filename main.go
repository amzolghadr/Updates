...
type App struct {
	...
	Channel     string `json:"channel,omitempty"` // github only: "" (stable/latest) or "alpha" (pre-releases included)
...
func checkGithub(repo, channel, githubToken string) (version, link string, err error) {
	...
	if channel == "alpha" {
		...
		// Repos that ship alpha/pre-release builds (e.g. ClashMetaForAndroid)
...
var (
	...
	farsroidTitleRe = regexp.MustCompile(`(?i)<title>(.*?)</title>`)
)
...
func checkFarsroid(pageURL string) (version, link string, err error) {
	...
	if m := farsroidVersionRe.FindStringSubmatch(html); m != nil {
		version = m[1]
	} else if m := farsroidTitleRe.FindStringSubmatch(html); m != nil {
		// Fallback: some pages (e.g. Poweramp) don't expose a clean version
		// number in the markup. Use the page <title> as a stand-in signal -
		// if the app gets updated, Farsroid usually edits the title too, so a
		// change here still means "go check it out", even without an exact
		// version string.
		version = strings.TrimSpace(m[1])
	} else {
		return "", "", fmt.Errorf("neither version nor title found on %s (site layout may have changed)", pageURL)
	}

	matches := farsroidLinkRe.FindAllStringSubmatch(html, -1)
	if len(matches) == 0 {
		// No scrapeable direct download link (e.g. it's loaded via JS on some
		// pages) - fall back to linking the Farsroid page itself.
		link = pageURL
	} else {
		best := matches[0][1]
